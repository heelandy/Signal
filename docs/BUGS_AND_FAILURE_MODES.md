...
| D5 ✅ | **Every open position stranded ("positions still open") — resolver dead since the first trail-eq row** | 15 rows sat 'open' overnight incl. a SPY trade whose TP1 price had printed; manual `track_outcomes` threw `database is locked` | THREE stacked defects: (1) `_walk_trail` used `np` but tracker.py **never imported numpy** — the first trail-eq row (2026-07-09 15:40) NameError'd EVERY resolver run, stranding all rows behind it; (2) the crash **leaked a mid-transaction sqlite handle** → jammed write lock → the "locked" symptom; (3) `get_bars` (5-10s network) ran INSIDE the open write transaction while the dashboard's 12s `/api/decisions` poll ran a CONCURRENT `track_outcomes` | numpy imported at module level; per-row try/except (one poisoned row never strands the rest); bars PREFETCHED before the write pass (ms-long lock); try/finally on every handle; `/api/decisions` + `/api/scorecard` made READ-ONLY — the scan cycle owns resolution. All 15 resolved on the next run (SPY trail +1.13R, an NQ tp2 +4R). Test `test_d5_tracker_resolver_cannot_be_stranded` |
