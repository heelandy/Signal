...
## F88 — LONGER-DTE OPTIONS (7 & 14 DTE): management helps, not yet a winner on 22 sessions (2026-07-08)
User 2026-07-08: 0DTE only — what about 1-2 week DTE, and how do we manage a credit spread now the
Alpaca feed is wired? `research/opra_longer_dte.py` on the OPRA chain (expiries out to 30d, daily-
close snapshots, settle at the expiry-date QQQ close), HOLD-to-expiry vs MANAGED (close at +50% of
credit on an intermediate day):
- **Credit spread 7/14 DTE HOLD: LOSES** (WR 59-61%, PF ~0.52, exp −0.16R) — more time = more
  directional risk, the short gets breached.
- **MANAGED (+50%): HELPS but still slightly negative** — 7DTE WR 59→73% PF 0.53→0.59; 14DTE WR
  61→72% PF 0.52→0.80, DD cut ~30%. The management lever (the wired feed's value) improves every
  metric; on 22 sessions it isn't profitable yet (a couple of tail days dominate the small N).
- **14DTE iron condor HOLD: POSITIVE hint** (WR 64%, PF 2.32, +0.47R, DD 1.0R) — wider-DTE condor
  breaches less; but n=14 (thin, not a verdict).
- naked_long "ref" is LOOKAHEAD (direction picked by the settle) — INVALID, ignore its numbers.
- **Verdict**: longer DTE + management is the RIGHT direction (metrics improve), but the 22-session
  window is too thin to confirm a winner. HOW TO MANAGE (answer): `native.walk_manage` = take profit
  at tp×credit, stop at stop×credit, else settle; LIVE = mark open positions each scan tick via
  `alpaca_chain_0dte`, apply TP/stop, close+journal. Infra wired; the per-tick live-position loop is
  the next step. A wider OPRA window is what firms this up.

## F87 — SMALL-ACCOUNT OPTIONS STRUCTURES: only the naked long works hold-to-settle (2026-07-08)
User 2026-07-08: the iron condor hits WR not PF (76.9%/1.28) and needs buying power; asked for a
small-account alternative, "if the strategy doesn't work find another." `research/opra_smallacct.py`
backtested three structures on the 22-session OPRA chain, HOLD-TO-SETTLE (no live feed to manage):
- **1 NAKED LONG (ATM 0DTE, directional by open lean): WR 36% · PF 1.34 · +0.218R · ~$300 cap —
  the ONLY positive structure.** Risk = premium; convex; = the options-0.1 NAKED lineage (F85
  PF 2.45). Low WR by design (does not fit the WR band; fits a convex profile).
- **2 CREDIT SPREAD ($3 wing, swept 0.3-1.0×EM): NEGATIVE at close strikes (PF 0.5), breakeven at
  1.0×EM (WR 77% but PF 1.01).** Premium selling does NOT work hold-to-settle — the tail eats it;
  it needs intraday TP/stop management (a live feed). The panel's earlier credit_spread sold far
  OTM for $0.12 credit / $5.88 risk — the strike bug that prompted this.
- **3 DEBIT SPREAD: breakeven (−0.04R, PF 0.95).**
- **Verdict**: for a small account, the NAKED LONG is the honest winner (positive, risk=premium
  ~$78-300/contract live). Premium-selling structures need the live Alpaca feed to manage. NO
  structure hits WR 75-85 AND PF 1.6-1.8 — the same WR/PF tension as the underlying geometry.
  WIRED: `native.build(structure="long_single")`, panel default + structure selector, live from
  the Alpaca chain (F86 feed). Settle math generalized (debit vs credit structures).

## F86 — OPTIONS-NATIVE STRATEGY FROM SCRATCH: 0DTE VRP condor — WR/DD in band, PF short (2026-07-08)
User 2026-07-08: build ONE options-only strategy (NOT a translation of any underlying signal) to
WR 75-85 · PF 1.6-1.8 · maxDD ≤11% · 1-2 signals/session; "work with what we have until results
are produced; no amount of data fixes a bad calibration." `research/opra_condor.py` + shared
`bot/options/native.py`, judged on REAL OPRA bid/ask (F85 parquet, 22 QQQ sessions).
- **Thesis** (F85): 0DTE ATM IV ~38% vs ~20% realized → the expected move is too wide. SELL it,
  DEFINED-RISK (the naked straddle's −340% tail killed G3) + actively managed.
- **Spec `options-native-0.1` v0.2**: 0DTE iron condor, shorts at spot ±1.1×EM (EM = real ATM
  straddle), $6 wings, credit = shorts@bid − wings@ask, TP 0.6×credit, stop 2.5×credit, entry
  10:00 + 13:00 ET, gate |spot−open|>1.0×EM. TREND-DAY FALLBACK (v0.2): a one-sided credit spread
  on the safe side (2.0×EM out, 1.25× stop) so the strategy fires EVERY session (strict min-1).
- **CALIBRATION ARTIFACT FOUND & FIXED**: an early result read WR 82 · PF 1.71 in-band — but the
  research loader filtered strikes to ±3.5% of spot, which silently DROPPED the 2.0×EM directional
  strikes (and some wings) on big-EM days, skipping hard losing trades. Building the LIVE journal
  path (full ±6% strikes) exposed the gap; `load_0dte` now loads the full window. The user's own
  warning ("no amount of data fixes a bad calibration") caught in practice.
- **HONEST result (full-strike, managed, 41 trades / 22 sessions): WR 78 · PF 1.35 · maxDD 1.2% ·
  1.86 sig/session · 0 stand-aside.** WR and DD are IN BAND; **PF ~1.35 falls short of 1.6-1.8.**
  Grid over em/wing/tp/stop/dir with strict min-1: ZERO cells reach the PF band (best ~1.29). This
  is the expected 0DTE-condor profile — high WR, small wins, occasional max-loss (negative skew) —
  so PF is the binding constraint, fragile at n≈40. Hold-to-settle (no intraday marks) is worse
  (PF 1.34→ lower); a pure Black-Scholes proxy (no real chain) collapses to PF ~1.1 (0DTE skew).
- **Verdict**: NOT band-passing on honest data. The mechanism is sound and the VRP edge is real,
  but PF needs either better day-selection (skip big-move days — but that breaks min-1) or a wider
  OPRA window to firm up. WIRED anyway (sealed `options-native-0.1`, approvable, real-chain journal
  via `bot/options/native.record_session` → `/api/options_native`) so it accrues an HONEST forward
  record. NOT auto-laddered. Binding dependency for live trading: a REAL options feed (BS insufficient).

## F85 — OPRA OPTIONS STUDY: real premiums judge the options stack (2026-07-08)
`research/opra_extract.py` compressed the 13 GB OPRA QQQ chain (OPRA.PILLAR cbbo-1m, 22 sessions
2026-05-27..06-26) to a 158 MB parquet (`data/opra_qqq_cbbo.parquet`, 17.5M ATM-window quote
rows); `research/opra_study.py` ran the three goals on real bid/ask. Report →
`BOT/data/ml/reports/opra_study.json`.
- **G1 IV-TRUTH (verdict-grade, 264 real ATM solves).** Market ATM IV averaged **30.8%** vs the
  shipped flat **20%** (+10.8 pts). Term structure: **0-1DTE 38.1%** (p10-p90 22-60%), weekly
  23.5%. Realized vol 19.8% → market charges **1.56× realized** (VRP_K). Real 0DTE half-spread
  **$0.022 (1.8%)** — tighter than the modeled $0.03. → CALIBRATION SHIPPED: `pricing.default_iv(dte)`
  term structure + `calibrate_realized_iv()` (×1.56, floored at the term structure), wired into
  `live.py`, `/api/contract`, `/api/options`, `/api/exit_plan` (replaces every flat-0.20 / raw-
  realized-vol default). The whole options panel now prices at what the chain actually charges.
- **G2 NAKED verdict CONFIRMED on real inputs.** In-window canonical signals = N**1** (the ORB
  fires ~once in 22 sessions) → real-premium replay is anecdotal here (that lone 2026-06-05 short
  paid +379% and survived 2× spread). The real test: re-judge the full 287-trade NAKED replay at
  the OPRA-measured IV+spread — gate **PASS** (avg +0.44 ret-on-premium, PF 2.45, CI-lo +0.29,
  OOS +0.42, 8/9 yrs) and still PASS at **2× spread** (PF 2.22, CI-lo +0.25). Raising IV 20→38%
  roughly halved the modeled edge (PF 3.45→2.45) but did NOT kill it — options-0.1 NAKED holds,
  now honestly de-rated. ~39% WR (convex, judged on expectancy not the WR band, by design).
- **G3 VRP short straddle REFUTED on real premiums (this window).** Selling the ATM 0DTE straddle
  at real bids: avg **−4.8%**/session, PF 0.86, median +16.7% but worst day **−340%** — the tail
  ate the premium. 22 sessions is directional, but the VIX-proxy VRP candidate does NOT survive
  real 0DTE prices here. Status: parked, not deployable; revisit with a wider window.
- **Goal verdict.** The doc's literal bar (a real-premium expression clearing the WR/PF band +
  surviving spread-doubling) is NOT met on 22 sessions — real-premium N is too small and the WR
  band doesn't fit convex naked options. What this data DID deliver: a verdict-grade IV
  calibration (shipped), a full-N confirmation of the NAKED lineage under realistic pricing, and
  a real-premium refutation of VRP. Verdict-grade G2/G3 need a wider OPRA window (the options
  analog of the trades-schema purchase); the forward journal already accrues real option outcomes.
