...
def _utf8_stdout() -> None:
    """The Windows console is cp1252; a subprocess tail decoded with errors='replace' carries
    \\ufffd, which cp1252 can't encode — this crashed the 2-year intake mid-run at the step-3
    print (2026-07-08). Re-encode our own stdout as UTF-8 so any byte prints."""
...
def main(folder: str, train: bool = True):
    ...
    # 1) register (auto-label; folder scan dedups by content fingerprint)
    r = register(folder)                       # symbol=None -> pure auto-detect
    n_reg = r.get("registered", 1 if "error" not in r else 0)
    print(f"1) registered: {n_reg} source(s)  ({r.get('duplicates', 0)} content-duplicates skipped)",
          flush=True)
    if "error" in r:
        print("   ", r["error"]); return

    # 2) synthesize — one subprocess per pending source (the OOM lesson: never accumulate).
    # Only 'registered' sources: duplicates and unknown-format rows are excluded here.
    pend = [x for x in _load() if x.get("status") == "registered"]
