...
# The project's THREE signal categories, in canonical order (user 2026-07-08):
#   1 equities (eq)  2 futures (ft)  3 options (op)
# Every journal row carries its `cat` so the shared journal stays one store while training and the
# scorecards can slice by category. `CATEGORY_ID` maps to the user-facing id-eq / id-ft / id-op.
CATEGORY_ORDER = ("eq", "ft", "op")
CATEGORY_ID = {"eq": "id-eq", "ft": "id-ft", "op": "id-op"}
CATEGORY_LABEL = {"eq": "equities", "ft": "futures", "op": "options"}
_FUTURES_SYMS = {"NQ", "MNQ", "ES", "MES", "GC", "MGC"}


def asset_category(symbol: str | None, family: str | None = None) -> str:
    """Classify a signal into the project's 3 categories: 'op' (an options lineage — the family
    decides, since an options play trades a QQQ/SPY underlying), else 'ft' (futures) / 'eq'
    (equities) by the instrument. Order is CATEGORY_ORDER (eq -> ft -> op)."""
    fam = str(family or "")
    if fam.startswith("options-native-") or fam.startswith("options-"):
        return "op"
    sym = str(symbol or "").upper()
    a = ASSETS.get(sym)
    if a is not None:
        return "ft" if a.is_futures else "eq"
    return "ft" if sym in _FUTURES_SYMS else "eq"


def category_sort_key(symbol: str | None, family: str | None = None) -> int:
    """Sort helper so any listing renders equities, then futures, then options."""
    return CATEGORY_ORDER.index(asset_category(symbol, family))
