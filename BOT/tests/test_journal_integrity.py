...
def test_clean_geometry_passes():
    ...
    # worker single-target: tp2=None (one tight target) is VALID
    assert _levels_ok(_sig(tp1=100.4, tp2=None)) is None
...
def test_impossible_geometry_rejected():
    ...
    assert _levels_ok(_sig(tp1=100.4, tp2=100.4)) is not None       # TP1==TP2 is corruption (user 2026-07-08)
...
def test_integrity_flags_planted_corruption(monkeypatch):
    ...
    rows = [
        ...
        ("cccccc3", "SPY", "short", "worker-s", bar, 745.88, 747.0, 745.51, None, '{"tf":"5m"}'),    # single-target cross-lineage: OK
...
def test_integrity_clean_when_lineages_differ(monkeypatch):
    ...
    rows = [
        ...
        ("a3", "SPY", "short", "worker-s", bar, 745.88, 747.0, 745.51, None, '{"tf":"5m"}'),
    ]
    monkeypatch.setattr(T, "_con", lambda: FakeCon(rows))
    rep = T.integrity()
