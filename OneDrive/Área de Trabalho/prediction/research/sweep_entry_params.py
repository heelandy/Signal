...
def run_combo(d, ctx_up, ctx_dn, p: dict) -> pd.DataFrame:
    ...
    return B.backtest(d, "tp2_full", "both", False, "orb", 0, T1, T2, ORS, ORE, 0.0, CUT, "close",
                      ...
                      watch_live=True, cooldown_bars=p["cooldown"],
                      stale_bars=p["stale"], retest_atr=p["retest"])
...
def sweep_symbol(sym: str, combos: list[dict]) -> dict:
    d = load(sym)
    ...
    return {"cutoff": str(cutoff)[:10], "min_is_trades": MIN_IS_TRADES,
            ...
            "used": {"symbol": sym, "timeframe": "5m", "session": "rth",
                     "bars": int(len(d)),
...
def main(syms: list[str], quick: bool = False) -> dict:
    combos = grid(quick)
    prev = {}
    ...
    for sym in syms:
        print(f"\n=== SWEEP {sym} ({len(combos)} combos + default) ===", flush=True)
        try:
            out["symbols"][sym] = sweep_symbol(sym, combos)
