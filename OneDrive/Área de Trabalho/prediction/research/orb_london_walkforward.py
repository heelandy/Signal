#!/usr/bin/env python3
"""
HIGHSTRIKE F29 confirm — WALK-FORWARD the London-session STACK (graduate it like Asia, F22b).
London 5m stack (st_state gate + VWAP-cap k2) on the 03:00-03:30 ET open window passed the basic gate
(+0.574R, PF 3.45, 17/17 yrs). Gate it fully: per-year + 70/30 OOS + 2x/3x slippage + ES cross-check.

    python research/orb_london_walkforward.py
"""
import sys, os
...
WINDOWS = [("03:00-03:30", 540, 570, 840), ("03:00-04:00", 540, 600, 840), ("02:00-02:30", 480, 510, 840)]
...
def main():
    ...
    for sym in ("NQ", "ES"):
        d = state(sym, "5m")
        print(f"\n################  {sym} 5m STACK — London walk-forward  ################")
