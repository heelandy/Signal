...
'Research over probabilistic area'). RESEARCH SIDE ONLY — no STACK/BOT propagation yet (user
instruction); when it graduates, STACK + BOT are the primary propagation targets.
...
recovery efficiency). Candidate/confirm respect the noise-control rule (2 of last 3 evaluations).
    ...
    python research/orb_liquidity_zones.py --selftest          # synthetic checks (run anywhere)
    python research/orb_liquidity_zones.py NQ ES               # data-drive: zone hit-rate vs
